...
void HardwareSerial::_rx_complete_irq(void)
{
  ...
  if (ssr.BIT.ORER == 0 && ssr.BIT.FER == 0 && ssr.BIT.PER == 0) {
    _store_char(c);
  } else {
    ssr.BIT.ORER = 0;
    ssr.BIT.FER = 0;
    ssr.BIT.PER = 0;
    ssr.BIT.b7_6 = 0b11;
    while (((volatile st_sci0_ssr*)&_sci->SSR)->BIT.ORER) {
      _sci->SSR.BYTE = ssr.BYTE;
    }
    while (((volatile st_sci0_ssr*)&_sci->SSR)->BIT.FER) {
      _sci->SSR.BYTE = ssr.BYTE;
    }
    while (((volatile st_sci0_ssr*)&_sci->SSR)->BIT.PER) {
      _sci->SSR.BYTE = ssr.BYTE;
    }
  }
#endif/*__RX600__*/
