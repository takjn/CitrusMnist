...
#if 0 // とりあえずtone()はソフトウェアPWMを使う
	if (isHardwarePWMPin(_pin)) {
		int period = HardwarePWMFreq / frequency;
		int term = period / 2;
		int length = (long long)HardwarePWMFreq * duration / 1000;
		setPinModeHardwarePWM(_pin, period, term, length);
	} else 
#else
	{
		int period = SoftwarePWMFreq / frequency;
		int term = period / 2;
		int length = (long long)SoftwarePWMFreq * duration / 1000;
